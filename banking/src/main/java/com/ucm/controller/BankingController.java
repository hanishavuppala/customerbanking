...
public class BankingController {
	...
	public String makeDeposit(@ModelAttribute("BankUser") BankUser bankUser, Model model,@RequestParam("id") String accountNumber) {
		model.addAttribute("accountNumber", accountNumber);
		return "deposit";
	}
	
	@RequestMapping(value = "/makeWithdraw", method = RequestMethod.GET)
	public String withdraw(@ModelAttribute("BankUser") BankUser bankUser, Model model,@RequestParam("id") String accountNumber) {
		model.addAttribute("accountNumber", accountNumber);
		return "withdraw";
	}
	
	@RequestMapping(value = "/doDeposit", method = RequestMethod.POST)
	public String doDeposit(@ModelAttribute("BankUser") BankUser bankUser, Model model,@RequestParam("accountNumber") String accountNumber) {
		boolean isDeposited = bankingService.doDeposit(accountNumber,bankUser.getDepositAmount());
		if(isDeposited) {
			bankUser.setAccountNumber(Integer.parseInt(accountNumber));
			BankUser updatedBankUser = bankingService.searchAccount(bankUser);
			model.addAttribute("updatedBalance", "Deposit Successfull current balance in your account is "+updatedBankUser.getAccountBalance()+" USD");
		}else {
	...
	public String doWithDraw(@ModelAttribute("BankUser") BankUser bankUser, Model model,@RequestParam("accountNumber") String accountNumber) {
		bankUser.setAccountNumber(Integer.parseInt(accountNumber));
		BankUser bankUser1 = bankingService.searchAccount(bankUser);
		
		if(bankUser1.getAccountBalance() > bankUser.getWithdrawAmount()) {
			boolean isWithdrawn = bankingService.doWithdraw(accountNumber,bankUser.getWithdrawAmount(),bankUser1.getAccountBalance());
			if(isWithdrawn) {
				BankUser withdrawnBalance = bankingService.searchAccount(bankUser);
				model.addAttribute("withdrawn", "Withdrawal successfull current balance in account is "+withdrawnBalance.getAccountBalance()+" USD");
			}else {
